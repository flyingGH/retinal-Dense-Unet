...
class SegmentionTrainer(TrainerBase):
	...
	def init_callbacks(self):
		self.callbacks.append(
			ModelCheckpoint(
				filepath=self.config.hdf5_path+self.config.exp_name+self.config.preprocess+'_'+self.config.dataset+'_last_weights.h5',
		        verbose=1,
		        monitor='val_loss',
		        mode='auto',
		        save_best_only=True
	...
	def train(self):
		...
		self.model.save_weights(self.config.hdf5_path+self.config.exp_name+self.config.preprocess+'_'+self.config.dataset+'_last_weights.h5', overwrite=True)
